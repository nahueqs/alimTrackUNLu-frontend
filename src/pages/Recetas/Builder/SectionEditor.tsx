    ...
    >
      <Space direction="vertical" style={{ width: '100%' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f0f2f5', padding: '8px', borderRadius: '4px' }}>
            ...
            <Button size="small" type="primary" icon={<PlusOutlined />} onClick={onAddField}>Agregar Campo</Button>
        </div>
                ...
                {section.campos.map(campo => (
                    <div key={campo.id} style={{ display: 'flex', gap: '8px', alignItems: 'center', padding: '8px', background: '#fff', border: '1px solid #d9d9d9', borderRadius: '4px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                            <Typography.Text type="secondary" style={{ fontSize: '12px' }}>Tipo:</Typography.Text>
                            <select 
                                value={campo.tipoDato}
                                onChange={(e) => onUpdateField(campo.id, { tipoDato: e.target.value as TipoDatoCampo })}
                                style={{ padding: '4px', borderRadius: '4px', border: '1px solid #d9d9d9', height: '32px' }}
                            >
                        ...
                        <Button danger icon={<DeleteOutlined />} onClick={() => onRemoveField(campo.id)} />
                    </div>
                ))}
            </div>
        ) : (
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f0f2f5', padding: '8px', borderRadius: '4px' }}>
            ...
            <Button size="small" type="primary" icon={<PlusOutlined />} onClick={onAddGroup}>Agregar Grupo</Button>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f0f2f5', padding: '8px', borderRadius: '4px' }}>
            ...
            <Button size="small" type="primary" icon={<PlusOutlined />} onClick={onAddTable}>Agregar Tabla</Button>
        </div>
